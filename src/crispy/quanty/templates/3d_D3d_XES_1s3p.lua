...
if CrystalFieldTerm then
    ...
    --
    -- The five 3d orbitals split into a1g + 2 eg. The two eg pairs descend from
    ...
    Akm = {{2, 0, 1}, {4, 0, 9 / 5}}
    Ea1g_3d = NewOperator("CF", NFermions, IndexUp_3d, IndexDn_3d, Akm)

    Akm = {{2, 0, 1}, {4, 0, -12 / 5}}
    Eegsigma_3d = NewOperator("CF", NFermions, IndexUp_3d, IndexDn_3d, Akm)

    Akm = {{2, 0, -2}, {4, 0, 3 / 5}}
    Eegpi_3d = NewOperator("CF", NFermions, IndexUp_3d, IndexDn_3d, Akm)

    Akm = {{4, -3, -3 * sqrt(7 / 5)}, {4, 3, 3 * sqrt(7 / 5)}}
    Meg_3d = NewOperator("CF", NFermions, IndexUp_3d, IndexDn_3d, Akm)

    Ea1g_3d_i = $Ea1g(3d)_i_value
    Eegsigma_3d_i = $Eegsigma(3d)_i_value
    Eegpi_3d_i = $Eegpi(3d)_i_value
    Meg_3d_i = $Meg(3d)_i_value
    ...
    Ea1g_3d_m = $Ea1g(3d)_m_value
    Eegsigma_3d_m = $Eegsigma(3d)_m_value
    Eegpi_3d_m = $Eegpi(3d)_m_value
    Meg_3d_m = $Meg(3d)_m_value

    Ea1g_3d_f = $Ea1g(3d)_f_value
    Eegsigma_3d_f = $Eegsigma(3d)_f_value
    Eegpi_3d_f = $Eegpi(3d)_f_value
    Meg_3d_f = $Meg(3d)_f_value

    H_i = H_i + Chop(
          Ea1g_3d_i * Ea1g_3d
        + Eegsigma_3d_i * Eegsigma_3d
        + Eegpi_3d_i * Eegpi_3d
        + Meg_3d_i * Meg_3d)

    H_m = H_m + Chop(
          Ea1g_3d_m * Ea1g_3d
        + Eegsigma_3d_m * Eegsigma_3d
        + Eegpi_3d_m * Eegpi_3d
        + Meg_3d_m * Meg_3d)

    H_f = H_f + Chop(
          Ea1g_3d_f * Ea1g_3d
        + Eegsigma_3d_f * Eegsigma_3d
        + Eegpi_3d_f * Eegpi_3d
        + Meg_3d_f * Meg_3d)
end
...
-- Emission dipole operators for the radiative 3p to 1s decay.
...
local TCartesian = {Tx_3p_1s, Ty_3p_1s, Tz_3p_1s}
Tr_3p_1s = CalculateT(TCartesian, Epsr)
Tl_3p_1s = CalculateT(TCartesian, Epsl)

-- Available spectra and the operators required to calculate them.
SpectraAndOperators = {
    ["Emission"] = {Tx_3p_1s, Ty_3p_1s, Tz_3p_1s},
    ["Circular Dichroic"] = {Tr_3p_1s, Tl_3p_1s},
}
...
local T = {}
for Operator, _ in pairs(T_3p_1s) do
...
if CalculationRestrictions == nil then
    G_3p_1s = CreateSpectra(-H_f, T_3p_1s, Psis_m, {{"Emin", Emin}, {"Emax", Emax}, {"NE", NPoints}, {"Gamma", Gamma}, {"DenseBorder", DenseBorder}})
else
    G_3p_1s = CreateSpectra(-H_f, T_3p_1s, Psis_m, {{"Emin", Emin}, {"Emax", Emax}, {"NE", NPoints}, {"Gamma", Gamma}, {"Restrictions", CalculationRestrictions}, {"DenseBorder", DenseBorder}})
end

if ShiftSpectra then
    G_3p_1s.Shift(ZeroShift + ExperimentalShift)
end

-- Create a list with the Boltzmann probabilities, one for each operator-state spectrum.
local dZ_3p_1s = {}
for _ in ipairs(T_3p_1s) do
    for j in ipairs(Psis_m) do
        table.insert(dZ_3p_1s, dZ_m[j])
    end
end

-- Map each operator to its index in the (unordered) operators table.
local Ids = {}
for k, v in pairs(T_3p_1s) do
    Ids[v] = k
end
...
for Spectrum, Operators in pairs(SpectraAndOperators) do
    if ValueInTable(Spectrum, SpectraToCalculate) then
        ...
        if Spectrum == "Emission" then
            ...
            Gemi = GetSpectrum(G_3p_1s, SpectrumIds, dZ_3p_1s, #T_3p_1s, #Psis_m)
            SaveSpectrum(Gemi, Prefix .. "_emi", Gaussian, Lorentzian)
        end

        if Spectrum == "Circular Dichroic" then
            Gr = GetSpectrum(G_3p_1s, SpectrumIds[1], dZ_3p_1s, #T_3p_1s, #Psis_m)
            Gl = GetSpectrum(G_3p_1s, SpectrumIds[2], dZ_3p_1s, #T_3p_1s, #Psis_m)
