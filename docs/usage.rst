...
At the ESRF
-----------
Crispy is available on the NICE cluster. Make sure that you connect to the cluster using ``ssh -Y rnice.esrf.fr`` to enable X11 forwarding; note the ``-Y`` option.
...
Now the ``crispy`` command should be available; just type it in the terminal to start the program.
