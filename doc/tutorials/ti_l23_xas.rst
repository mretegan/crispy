...
broadening to 0.1 eV and the Gaussian broadening to 0.0 eV. Leave all the other
parameters at their default values. How many transitions are visible in the
...
parameters, i.e., the Slater integrals and the spin-orbit coupling constants,
...
affects the energy separation between the |L2| and |L3| edges the most? Check
that the energy separation between the two edges is close to 3/2·ζ(2p).

3. Change the scaling parameter back to 1.0 for both the 2p and 3d spin-orbit
...
the intensity ratio of the |L3| and |L2| edges is close to the statistical
value of 2:1, which corresponds to a branching ratio, defined as
I(|L3|) / [I(|L3|) + I(|L2|)], of 2/3.
...
given by the degeneracies of the |3d(t2g)| and |3d(eg)| orbitals (3:2) and the
|L3|/|L2| intensity ratio discussed before (2:1). What is the energy separation between the first two
transitions? How does this compare to the energy separation between the last
two transitions?

Adding Tetragonal Distortion
----------------------------
1. Next, we are going to study the influence of a tetragonal distortion, i.e.,
an elongation or compression along one of the four-fold axes. Lowering the
...
2. Change the symmetry of the system to |D4h|. Note that by doing this the
crystal field parameters are reset to their default values, while most other
parameters, including the scale factors, are kept. Set the scaling of the
Slater integrals and of the 3d spin-orbit coupling back to 0.8 and 1.0,
respectively. Set the Dq value to 0.25 eV. This is equivalent to setting the
10Dq value to 2.5 eV in the case of the |Oh| symmetry. While keeping Dt zero, vary the value of Ds between -0.6 and 0.6 eV
