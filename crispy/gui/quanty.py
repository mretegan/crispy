...
###################################################################
"""This module provides Quanty functionality."""
...
class Spectrum2D(Spectrum):
    ...
    def plot(self, plotWidget=None):
        ...
        plotWidget.setGraphXLabel(self.xLabel)
        plotWidget.setGraphYLabel(self.yLabel)
        plotWidget.addImage(self.z, origin=self.origin, scale=self.axesScale)
...
class QuantySpectra(object):
    ...
    def toCalculateChecked(self, values):
        ...
        for spectrum in values:
            if spectrum in ("Isotropic", "Circular Dichroism", "Linear Dichroism"):
                spectraNames.append(self.aliases[spectrum])
    ...
    def loadFromDisk(self, calculation):
        ...
        for spectrumName in self.toPlot:
            ...
            if calculation.experiment in ["XAS", "XPS", "XES"]:
                ...
                self.broadenings = {
                    "gaussian": (calculation.xGaussian,),
                }
            else:
                xMin = calculation.xMin
                xMax = calculation.xMax
                ...
                self.broadenings = {
                    "gaussian": (calculation.xGaussian, calculation.yGaussian),
                }
...
class QuantyCalculation(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.baseName = "{}{}_{}_{}_{}".format(
            self.element, self.charge, self.symmetry, shortEdge, self.experiment
        )
        ...
        self.spectra.toCalculateChecked = [
            "Isotropic",
        ]
        ...
        branch = self._parameters["elements"][self.element]["charges"][
            self.charge
        ]  # noqa
    ...
    def saveInput(self):
        templatePath = resourceFileName(
            "quanty:" + os.path.join("templates", "{}".format(self.templateName))
        )
        ...
        for replacement in replacements:
            self.input = self.input.replace(replacement, str(replacements[replacement]))
...
class QuantyDockWidget(QDockWidget):
    ...
    def populateWidget(self):
        ...
        self.symmetryComboBox.setItems(self.state._symmetries, self.state.symmetry)
        self.experimentComboBox.setItems(self.state._experiments, self.state.experiment)
        ...
        self.spectraModel.setModelData(
            self.state.spectra.toCalculate, self.state.spectra.toCalculateChecked
        )
        self.spectraModel.checkStateChanged.connect(self.updateSpectraCheckState)
        ...
        if not hasattr(self, "resultsModel"):
            ...
            self.resultsModel.itemNameChanged.connect(self.updateCalculationName)
            self.resultsModel.itemCheckStateChanged.connect(self.updatePlotWidget)
    ...
    def activateWidget(self):
        ...
        self.experimentComboBox.currentTextChanged.connect(self.resetState)
        self.edgeComboBox.currentTextChanged.connect(self.resetState)

        self.temperatureLineEdit.returnPressed.connect(self.updateTemperature)
        self.magneticFieldLineEdit.returnPressed.connect(self.updateMagneticField)
        ...
        self.eps11LineEdit.returnPressed.connect(self.updateIncidentPolarizationVectors)
        ...
        self.yLorentzianLineEdit.returnPressed.connect(self.updateYLorentzian)
        ...
        self.nConfigurationsLineEdit.returnPressed.connect(self.updateConfigurations)
    ...
    def updateXMin(self):
        ...
        if xMin > self.state.xMax:
            message = "The lower energy limit cannot be larger than " "the upper limit."
            self.getStatusBar().showMessage(message, self.timeout)
            self.xMinLineEdit.setValue(self.state.xMin)
            return
    ...
    def updateXMax(self):
        ...
        if xMax < self.state.xMin:
            message = (
                "The upper energy limit cannot be smaller than " "the lower limit."
            )
            self.getStatusBar().showMessage(message, self.timeout)
            self.xMaxLineEdit.setValue(self.state.xMax)
            return
    ...
    def updateXNPoints(self):
        ...
        if xNPoints < xNPointsMin:
            message = "The number of points must be greater than " "{}.".format(
                xNPointsMin
            )
            self.getStatusBar().showMessage(message, self.timeout)
            self.xNPointsLineEdit.setValue(self.state.xNPoints)
    ...
    def updateXLorentzian(self):
        ...
        try:
            ...
            if not xMin < xLorentzianPivotEnergy < xMax:
                message = (
                    "The transition point must lie between the upper "
    ...
    def updateIncidentPolarizationVectors(self):
        ...
        k1 = self.state.k1
        if np.dot(np.array(k1), np.array(eps11)) != 0:
            message = "The wave and polarization vectors need to be " "perpendicular."
            self.getStatusBar().showMessage(message, self.timeout)
            self.eps11LineEdit.setVector(self.state.eps11)
            return
    ...
    def updateConfigurations(self, *args):
        ...
        if nConfigurations > self.state.nConfigurationsMax:
            ...
            self.getStatusBar().showMessage(message, self.timeout)
            self.nConfigurationsLineEdit.setValue(self.state.nConfigurationsMax)
    ...
    def saveInput(self):
        ...
        try:
            ...
            message = (
                "The specified folder doesn't exist. Use the 'Save "
    ...
    def saveInputAs(self):
        path, _ = QFileDialog.getSaveFileName(
            self,
            "Save Quanty Input",
            os.path.join(self.getCurrentPath(), "{}.lua".format(self.state.baseName)),
    ...
    def saveAllResultsAs(self):
        path, _ = QFileDialog.getSaveFileName(
            self,
            "Save Results",
            os.path.join(self.getCurrentPath(), "{}.pkl".format("untitled")),
            "Pickle File (*.pkl)",
        )
    ...
    def loadResults(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Load Results", self.getCurrentPath(), "Pickle File (*.pkl)"
        )
    ...
    def runCalculation(self):
        ...
        with open(os.devnull, "w") as f:
            try:
                ...
                if e.errno == errno.ENOENT:
                    message = (
                        "The Quanty executable is not working "
        ...
        message = 'Running "Quanty {}" in {}.'.format(
        ...
        for result in results:
            if isinstance(result, ExperimentalData):
                ...
                for spectrum in result.spectra.processed:
                    spectrum.legend = "{}-{}".format(result.index, spectrum.shortName)
    ...
    def loadExperimentalData(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Load Experimental Data", self.getCurrentPath(), "Data File (*.dat)"
        )
        ...
        if path:
            ...
            if result.spectra is not None:
                ...
                message = (
                    "Failed to read experimental data. Please check "
...
class QuantyPreferencesDialog(QDialog):
    ...
    def saveSettings(self):
        ...
        self.settings.setValue("RemoveFiles", self.removeFilesCheckBox.isChecked())
...
class QuantyResultDetailsDialog(QDialog):
    ...
    def activateWidget(self):
        ...
        self.normalizationComboBox.currentTextChanged.connect(self.updateNormalization)
    ...
    def populateWidget(self):
        ...
        self.normalizationComboBox.setCurrentText(self.state.spectra.normalization)
    ...
    def updateBroadening(self):
        ...
        if self.state.experiment in ["RIXS"]:
            ...
            broadenings = {
                "gaussian": (self.state.xGaussian, self.state.yGaussian),
                "lorentzian": (self.state.xLorentzian, self.state.yLorentzian),
            }
        else:
            broadenings = {
                "gaussian": (self.state.xGaussian,),
                "lorentzian": (self.state.xLorentzian,),
            }
